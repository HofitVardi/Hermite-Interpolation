...
def create_fig_(k):
    ...
    if (k==10):  ###  Figure 10  ###
        n = 101
        res = []
        for x in np.linspace(0,0.75*np.pi,n):
            for y in np.linspace(0,0.75*np.pi,n):
                for z in np.linspace(0,np.pi,n):
                    if (x**2+y**2+z**2>0):
                        if((x**2+y**2)**0.5<=3*np.pi/4):
                            if (z+x>=y and z+y>=x and x+y>=z):
                                c = (x+y)/4
                                theta00 = 4*cos(c)**2*cos(x)+1-cos(z)
                                theta00 = theta00/(16*cos(c)**4+2-2*cos(z)+8*cos(c)**2*(cos(x)-cos(y)))**0.5
                                theta01 = (12*cos(c)**2+cos(x)-5*cos(y))*cos(c)**2
                                theta01 = theta01/(8*cos(c)**4+1-cos(z)+4*cos(c)**2*(cos(x)-cos(y)))**0.5
                                theta01 = theta01/(18*cos(c)**4+1+cos(z)-6*cos(c)**2*(cos(x)+cos(y)))**0.5
                        
                                if(theta00<=1 and theta00>=-1):
                                    if(theta01<=1 and theta01>=-1):
                                        d = 0.9*(x**2+y**2)-(arccos(theta00)**2+arccos(theta01)**2)
                                        q = (arccos(theta00)**2+arccos(theta01)**2)/(x**2+y**2)
                                        res.append([d,q,x,y,z])
                                    else:
                                        print("right="+str(theta01))
                                        print(x)
                                        print(y)
                                        print(z)
                                        print()
                                else:
                                    print("left="+str(theta00))
                                    print(x)
                                    print(y)
                                    print(z)
                                    print()
        ...
        fig = plt.figure()
        ax = Axes3D(fig)
        p = ax.scatter(x,y,z,c=D,cmap='pink')
        fig.colorbar(p,location="bottom",shrink=0.6,anchor=(0.5,1.8))
        plt.title("D over Omega")
        plt.show()
        
        fig = plt.figure()
        ax = Axes3D(fig)
        p = ax.scatter(x,y,z,c=Q,cmap='pink')
        fig.colorbar(p,location="bottom",shrink=0.6,anchor=(0.5,1.8))
        plt.title("Q over Omega")
        plt.show()